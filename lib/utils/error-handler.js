...
function errorHandler (er) {
  ...
  if (!npm.config || !npm.config.loaded) {
    ...
    console.error(replaceInfo(er.stack || er.message))
  }
  ...
  if (!er) return exit(0)
  if (typeof er === "string") {
    log.error("", replaceInfo(er))
    return exit(1, true)
  } else if (!(er instanceof Error)) {
    log.error("weird error", replaceInfo(er))
    return exit(1, true)
  }
  ...
  // Sanitize error message and stack to remove credentials from URLs
